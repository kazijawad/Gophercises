...
func main() {
	...
	mux.HandleFunc("/modify", func(w http.ResponseWriter, r *http.Request) {
		...
		nStr := r.FormValue("n")
		if nStr == "" {
			renderNumShapeChoice(w, r, f, ext, primitive.Mode(mode))
			return
		}
...
func renderNumShapeChoice(w http.ResponseWriter, r *http.Request, rs io.ReadSeeker, ext string, mode primitive.Mode) {
	opts := []genOpts{
		{N: 10, M: mode},
		{N: 20, M: mode},
		{N: 30, M: mode},
			...
			<body>
				{{range .}}
					<a href="/modify/{{.Name}}?mode={{.Mode}}&n={{.NumShapes}}">
						<img style="width: 20%;" src="/img/{{.Name}}"></img>
					</a>
				{{end}}
			</body>
			</html>
